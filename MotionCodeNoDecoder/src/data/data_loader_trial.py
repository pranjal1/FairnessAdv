...
class MotionDataLoader:
    def __init__(
        ...
        self.valid_train_test_combs = {'dws': [([1, 2], [11]), ([1, 11], [2])],
                                'ups': [([3, 4], [12]), ([3, 12], [4]), ([4, 12], [3])],
                                'wlk': [([7, 8], [15]), ([7, 15], [8])],
                                'jog': [([9], [16])],
                                'sit': [([5], [13])],
                                'std': [([6], [14])]}
        
        self.valid_combs_listed = []
        for vc in list(product(*self.valid_train_test_combs.values())):
            tmp_dct = {}
            for i,k in enumerate(self.tgt_labels):
                tmp_dct[k] = sum(vc[i], [])
            self.valid_combs_listed.append(tmp_dct)

        self.train_test_codes = self.valid_combs_listed[self.dataset_comb_number]
        print(self.train_test_codes)
    ...
    def load_data(self):
        ...
        for i, sub_id in tqdm(enumerate(ds_list[:, 0])):
            for j, activity in enumerate(self.tgt_labels):
                for trial in self.train_test_codes[activity]:
                    fname = (
                        self.folder_path
                        + activity
                        + "_"
                    ...
                    for slice_idx in range(
                        ...
                        if trial != self.train_test_codes[activity][-1]:
                            if gender == 0:
    ...
    def train_test_val_split(self):
        ...
        male_down_train_ratio = 0.9

        # (
        #     self.male_down_x_train,
        #     self.male_down_x_val,
        #     self.male_down_y_train,
        #     self.male_down_y_val,
        # ) = train_test_split(*male_down_all_data, test_size=1 - male_down_train_ratio)
        male_train_samples_count = len(self.male_train_data_samples)
        self.male_down_x_train = self.male_train_data_samples[male_train_samples_count//5:]
        self.male_down_x_val = self.male_train_data_samples[:male_train_samples_count//5]
        self.male_down_y_train = self.male_train_tgt_labels[male_train_samples_count//5:]
        self.male_down_y_val = self.male_train_tgt_labels[:male_train_samples_count//5]
        ...
        female_down_train_ratio = 0.9

        # (
        #     self.female_down_x_train,
        #     self.female_down_x_val,
        #     self.female_down_y_train,
        #     self.female_down_y_val,
        # ) = train_test_split(
        #     *female_down_all_data, test_size=1 - female_down_train_ratio
        # )

        female_train_samples_count = len(self.female_train_data_samples)
        self.female_down_x_train = self.female_train_data_samples[female_train_samples_count//5:]
        self.female_down_x_val = self.female_train_data_samples[:female_train_samples_count//5]
        self.female_down_y_train = self.female_train_tgt_labels[female_train_samples_count//5:]
        self.female_down_y_val = self.female_train_tgt_labels[:female_train_samples_count//5]
