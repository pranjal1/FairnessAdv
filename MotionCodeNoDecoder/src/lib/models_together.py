...
class TrainerModel(tf.keras.Model):
    ...
    def compute_loss(
        ...
        predict_target = self.adv_classifier_tt.model(z)
        predict_sensitive = self.adv_classifier_ts.model(z)

        male_tgt_pred = predict_target[target_sensitive == 1.0]
        female_tgt_pred = predict_target[target_sensitive == 0.0]

        male_sens_pred = predict_sensitive[target_sensitive == 1.0]
        female_sens_pred = predict_sensitive[target_sensitive == 0.0]

        loss_tt_male = self.adv_classifier_tt.calculate_loss(
            male_tgt_pred, target_label[target_sensitive == 1.0]
        )
        loss_tt_female = self.adv_classifier_tt.calculate_loss(
            female_tgt_pred, target_label[target_sensitive == 0.0]
        )

        loss_ts_male = self.adv_classifier_ts.calculate_loss(
            male_sens_pred, target_sensitive[target_sensitive == 1.0]
        )
        
        loss_ts_female = self.adv_classifier_ts.calculate_loss(
            female_sens_pred, target_sensitive[target_sensitive == 0.0]
        )

        loss_ts_male_opp = self.adv_classifier_ts.calculate_loss(
            male_sens_pred, tf.zeros_like(target_sensitive[target_sensitive == 1.0])
        )
        
        loss_ts_female_opp = self.adv_classifier_ts.calculate_loss(
            female_sens_pred, tf.ones_like(target_sensitive[target_sensitive == 0.0])
        )

        return loss_tt_male, loss_tt_female, loss_ts_male, loss_ts_female, loss_ts_male_opp, loss_ts_female_opp

    def get_trainable_vars(self):
        self.compute_loss(
            x=tf.random.normal(
                shape=(self.batch_size, self.time_length, self.data_dim), dtype=tf.float32
            ),
            ...
            target_sensitive=tf.constant(
                [0]*(self.batch_size//2)+[1]*(self.batch_size//2),
                dtype=tf.float32,
            ),
        )
