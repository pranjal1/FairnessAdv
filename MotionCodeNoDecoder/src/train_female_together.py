...
def run_train(train_config, data_dump_path, downstream_data_dump_path):
    ...
    x_train_female = downstream_data["female_x_train"]
    target_label_train_female = downstream_data["female_target_label_train"]
    sensitive_label_train_female = np.array([np.float32(0.0)] * len(x_train_female))
    ...
    tf_x_train_female = (
        tf.data.Dataset.from_tensor_slices(
            (
                x_train_female,
                target_label_train_female,
                sensitive_label_train_female,
            )
        )
        .shuffle(len(x_train_female))
        .batch(FLAGS.base_config.batch_size)
        .repeat()
    )
    ...
    all_results["female_train_original"] = {
        "male_test":male_res,
    ...
    # optimizer_new = tf.keras.optimizers.Adam(learning_rate=FLAGS.base_config.learning_rate//10)
        ...
        for i, (x_seq, tgt_label, sens_label) in enumerate(
            ...
            try:
                with tf.GradientTape() as enc_tt_tape, tf.GradientTape() as gender_tape:
                    ...
                    loss_gender = loss_ts_male + loss_ts_female
                    loss_enc_tt = 0.1 * loss_tt_female - loss_ts_female
                    # loss_enc_tt = 0.5 * loss_tt_female + loss_ts_female_opp
    ...
    num_split = np.ceil(len(x_train_female) / FLAGS.base_config.batch_size)
    female_x_train_batches = np.array_split(
        x_train_female, num_split, axis=0
    )
    female_downstream_y_train_pred = np.vstack(
        [model.adv_classifier_tt.model(model.encode(x_batch)).numpy() for x_batch in female_x_train_batches]
    )
    female_downstream_s_train_pred = np.vstack(
        [model.adv_classifier_ts.model(model.encode(x_batch)).numpy() for x_batch in female_x_train_batches]
    )
    ypred_tgt = np.argmax(female_downstream_y_train_pred, axis=-1)
    ypred_sens = np.argmax(female_downstream_s_train_pred, axis=-1)
    res_tgt = np.unique(target_label_train_female == ypred_tgt, return_counts=True)
    res_sens = np.unique(sensitive_label_train_female == ypred_sens, return_counts=True)
    ...
    all_results["female_train_w_adv"] = {
        "male_test":male_res,
        "female_test":female_res,
    }
    ...
    # num_split = np.ceil(len(downstream_data["male_x_test"]) / FLAGS.base_config.batch_size)
    # male_x_test_batches = np.array_split(
    #     downstream_data["male_x_test"], num_split, axis=0
    # )
    # num_split = np.ceil(len(downstream_data["male_x_train"]) / FLAGS.base_config.batch_size)
    # male_x_train_batches = np.array_split(
    #     downstream_data["male_x_train"], num_split, axis=0
    # )
    # num_split = np.ceil(len(downstream_data["male_x_val"]) / FLAGS.base_config.batch_size)
    # male_x_val_batches = np.array_split(
    #     downstream_data["male_x_val"], num_split, axis=0
    # )

    # male_downstream_z_test = np.vstack(
    #     [model.encode(x_batch).numpy() for x_batch in male_x_test_batches]
    # )
    # male_downstream_z_train = np.vstack(
    #     [model.encode(x_batch).numpy() for x_batch in male_x_train_batches]
    # )
    # male_downstream_z_val = np.vstack(
    #     [model.encode(x_batch).numpy() for x_batch in male_x_val_batches]
    # )

    # num_split = np.ceil(len(downstream_data["female_x_test"]) / FLAGS.base_config.batch_size)
    # female_x_test_batches = np.array_split(
    #     downstream_data["female_x_test"], num_split, axis=0
    # )
    # num_split = np.ceil(len(downstream_data["female_x_train"]) / FLAGS.base_config.batch_size)
    # female_x_train_batches = np.array_split(
    #     downstream_data["female_x_train"], num_split, axis=0
    # )
    # num_split = np.ceil(len(downstream_data["female_x_val"]) / FLAGS.base_config.batch_size)
    # female_x_val_batches = np.array_split(
    #     downstream_data["female_x_val"], num_split, axis=0
    # )

    # female_downstream_z_test = np.vstack(
    #     [model.encode(x_batch).numpy() for x_batch in female_x_test_batches]
    # )
    # female_downstream_z_train = np.vstack(
    #     [model.encode(x_batch).numpy() for x_batch in female_x_train_batches]
    # )
    # female_downstream_z_val = np.vstack(
    #     [model.encode(x_batch).numpy() for x_batch in female_x_val_batches]
    # )

    # np.savez(
    #     os.path.join(outdir, "downstream_z"),
    #     male_z_train=male_downstream_z_train,
    #     male_z_test=male_downstream_z_test,
    #     male_z_val=male_downstream_z_val,
    #     female_z_train=female_downstream_z_train,
    #     female_z_test=female_downstream_z_test,
    #     female_z_val=female_downstream_z_val,
    # )
